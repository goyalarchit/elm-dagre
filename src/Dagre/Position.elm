...
verticalAlignment rankList conflicts neighbourFn =
    let
        root =
            DE.fromListBy identity (List.concat rankList)
...
horizontalCompaction rankList sepFn root align =
    let
        ...
        shift =
            DE.fromListBy (\_ -> DU.infinity) (List.concat rankList)
...
positionY rankList rankSep =
    let
        ...
        ( _, ys_assigned ) =
            List.foldl (assignAbsoluteY rankSep 0) ( 0, ys ) rankList
    in
    ys_assigned
