module Render exposing (svgDrawNode,svgDrawEdge,draw)
...
import TypedSvg.Types exposing (Paint(..), AnchorAlignment(..), AlignmentBaseline(..), FontWeight(..)
                               , Length(..),Cursor(..),Transform(..),Display(..)
...
svgDrawEdge _ (sourceX,sourceY) (targetX,targetY) controlPts =
    polyline    [ points <| List.concat [[(sourceX,sourceY)], controlPts, [(targetX,targetY)]]
...
svgDrawNode node (posX,posY) =
    g
        ...
        [ TS.circle    [ r 16
                    ...
                    , cy posY ] 
                    []
        , TS.text_  [ textAnchor AnchorMiddle 
...
nodeDrawing node_ drawNode_ coordDict =
    let
        pos = Maybe.withDefault (-10,-10) (Dict.get node_.id coordDict)
    in
        drawNode_ node_ pos
...
edgeDrawing edge_ drawEdge_ coordDict controlPointsDict =
    let
        getCoords = \p -> Maybe.withDefault (-10,-10) (Dict.get p coordDict) 
        sourcePos = getCoords edge_.from

        targetPos = getCoords edge_.to
        ctrlPts = Maybe.withDefault [] (Dict.get (edge_.from,edge_.to) controlPointsDict) |> List.map getCoords
    in
        drawEdge_ edge_ (sourcePos) (targetPos) ctrlPts



getCoordDict : Graph n e -> (Dict.Dict G.NodeId (Float,Float),Dict.Dict (G.NodeId,G.NodeId) (List G.NodeId))
getCoordDict graph =
    D.runLayout graph
...
getCanvasSize coordDict =
    let
        coords = Dict.values coordDict
        xCoords = List.map (\(x,_) -> x) coords
        yCoords = List.map (\(_,y) -> y) coords
        minX = (Maybe.withDefault -100.0  (List.minimum xCoords)) - 100
        minY = (Maybe.withDefault -100.0  (List.minimum yCoords)) - 100
        maxX = Maybe.withDefault 200.0  (List.maximum xCoords)

        maxY = Maybe.withDefault 400.0  (List.maximum yCoords)

    in
        ( ( minX , minY ) , ( maxX - minX + 100 , maxY - minY + 100 ) )


draw : NodeDrawer n (Svg msg) -> EdgeDrawer e (Svg msg) -> Graph n e -> (Svg msg)
draw drawNode drawEdge graph = 
    let 
        (coordDict,controlPointsDict) = getCoordDict graph
        ((minX,minY),(w,h)) = getCanvasSize coordDict
        edgesSvg =
